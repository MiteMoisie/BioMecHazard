...
package biomechazard.world.meta;
		...
		return table -> {
			...
			for (T t : orderedKeys) {
				...
				table.table(bt -> {
					...
					if (type instanceof CanisterBulletType types) {
						if (types.fragBullet != null) {
							if ((types.fragBullet instanceof MineBulletType)) {
								sep(bt, Core.bundle.format("bullet.biomechazard-mineCount", types.fragBullets));
							} else {
								sep(bt, Core.bundle.format("bullet.biomechazard-burstCount", types.fragBullets));
							}
							bt.row();
					...
					// Frag Bullets | Replaces Vanilla Frag Bullets
					if (type.fragBullet != null && !(type instanceof CanisterBulletType) && !(type.fragBullet instanceof AftershockBulletType)) {
						sep(bt, Core.bundle.format("bullet.frags", type.fragBullets));
						bt.row();
						ammo(ObjectMap.of(t, type.fragBullet), indent + 1, false).display(bt);
					} else if (type.fragBullet instanceof AftershockBulletType splashBulletType) {
						sep(bt, Core.bundle.format("bullet.biomechazard-aftershock", splashBulletType.splashAmount));
						bt.row();
						ammo(ObjectMap.of(t, type.fragBullet), indent + 1, false).display(bt);
					}
